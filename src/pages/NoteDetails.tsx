...
import { IconClock, IconTag } from "@/lib/icons";
  ...
  return (
    <>
      ...
      <div className="flex lg:hidden items-center justify-between w-full py-2">
        <Link to={`/`} className="flex items-center gap-1 pl-[14px] text-black">
          <img
            src={chevronLeftIcon}
...
{/* -------------- Note Body ------------------ */}
      <section
        className={`${
          location.pathname === "/" ? "hidden lg:block" : "block"
      ...
      >
        ...
        <article className="w-full gap-8 flex items-center pb-5 mb-2 border-b-[1px] border-[#E0E4EA]">
          ...
          <div className="pt-5 flex flex-col gap-2">
            ...
            <p className="text-[#2B303B] font-normal pt-2 text-sm tracking-[-0.2px]">{note.lastEdited.split("T")[0]}</p>
          </div>
        </article>
